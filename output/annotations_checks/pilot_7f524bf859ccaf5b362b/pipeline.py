...
def _run_annotations(run_dir: Path, execute: bool, workers: int,
    ...
    if execute and API_POLICY_PATH.exists():
        ...
        if policy.get('allow_api_calls') is not True:
            grant = policy.get('authorized_runs', {}).get(run_dir.name, {})
            spec = manifest['spec']
            if (grant.get('model') != spec['model']
                    or grant.get('reasoning_effort') != spec['reasoning_effort']
                    or grant.get('run_dir') != str(run_dir.resolve())
                    or type(grant.get('max_calls')) is not int
                    or not 0 < manifest['sample_size'] <= grant['max_calls']):
