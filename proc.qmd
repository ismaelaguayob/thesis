# Metodología: Discourse Network Análisis

## Variables:

- **Actores:** Organizaciones o sujetos. Deben definirse de forma clara antes de empezar a analizar.

- **Conceptos:** puede ser tanto afirmaciones sobre políticas (perspectiva de advocacy coalitions) o justificaciones/narrativas (probablemente más adecuado en nuestro caso de estudio).

- **Acuerdo: acuerdo o desacuerdo**

- **Tiempo:** Puede definirse de forma discreta o continua, dependiendo de si se van a utilizar técnicas inferenciales más complejas.

Permite estudiar las coaliciones y su transformación a lo largo del tiempo. Esto posibilita evaluar el nivel de cooperación, el grado de polarización, los ciclos de atención a los problemas sociales, y define una forma sistemática de analizar como el poder le da forma al discurso.

Hay diferentes formas de codificar (se ajustan a la teoría y al problema) y aún debo estudiar los métodos avanzados inferenciales del análisis de redes que esta perspectiva emplea. Pero la forma más básica es así:

**Actor -\> acuerdo -\> concepto (en un time step)**
...
Johannes Kayser -\> Acuerdo -\> El Estado se roba el dinero

En último término, esto permite construir grafos y analizar el dinamismo de las coaliciones (aún sin pasar a métodos complejos), como en este ejemplo del análisis del discurso en torno a las pensiones en alemania:
...
# Filtrado y descarga de trámites irrelevantes
...
Extraemos el discurso desde los diarios de sesión, dividido por párrafo y separado por hablante. Los datos ya están en su gran mayoría identificados, ya que estamos utilizando los identificadores de hablantes y tipos de intervenciones presentes en el Akoma Ntoso de la Biblioteca del Congreso Nacional. Para esto, utilizamos la función `add_speech_content`.

Sin embargo, parte del discurso no está etiquetado, principalmente el discurso de los ministros, que no son parlamentarios, y por lo tanto no están etiquetados como hablantes en el Akoma Ntoso. Para esto, se hace un proceso de normalización del discurso, donde se identifican los fragmentos de texto que corresponden a cada ministro, y se les asigna manualmente un identificador de hablante específico para cada uno, generando el mismo esquema para ambas participaciones. Para esto, se utiliza la función `normalize_speech_content`.
...
parliamentarians_debug = debug_parliamentarian_data_errors(
    parliamentarians,
...
parliamentarians_debug.to_parquet("data/parliamentarians.parquet")
...
#file_name = Path("data/datos_normalizados.json")

#with open(file_name, 'w', encoding='utf-8') as f:
#    json.dump(dict_normalized, f, ensure_ascii=False, indent=4)
```
