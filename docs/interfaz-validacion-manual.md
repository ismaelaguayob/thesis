...
La fuente editable activa está en `features/codebook/codebook_v0.3.xlsx`. Las versiones 0.1 y 0.2 se conservan sin cambios para reconstruir las rondas anteriores. Todas contienen las hojas `README`, `Metadatos` y `Conceptos`.
...
El JSON `features/codebook/codebook_v0.3.json` es un archivo derivado y no debe editarse directamente. Se genera o comprueba con:
...
Una declaración que requiere ampliar el libro usa `concept_status = review`, `concept_id = null` y un `proposed_concept` no vacío.
