...
| Características políticas y personales | `party_at_date`, estado y fuente de la afiliación histórica, género y fecha de nacimiento disponibles en los metadatos |

La clasificación en grupos políticos utiliza la afiliación correspondiente a la fecha del debate. `proc.qmd` enlaza `parliamentarian_affiliations.parquet` mediante documento y persona y conserva `party_at_date`, el estado del emparejamiento y la fuente. `current_party` permanece como dato de procedencia. `party_at_date_status` distingue afiliación encontrada, afiliación no aplicable para funciones institucionales o ejecutivas y afiliación desconocida. Los parlamentarios con resultados `not_found` o `ambiguous` se mantienen como afiliación faltante hasta su resolución manual. La tabla de equivalencias de partidos e independientes se conserva junto con la base analítica.
...
Las rondas de calibración manual y los pilotos preceden a la evaluación de una muestra estratificada. El diseño garantiza cobertura por ley y cámara y conserva la afiliación histórica, el género y el tipo de actor para estudiar la distribución de los errores. Antes de sortear la muestra definitiva se fijarán la unidad primaria y la forma de incorporar partido y género, ya sea mediante cruces, cuotas marginales o análisis posterior, para evitar estratos con cuota cero. La selección conserva la trazabilidad de cada fragmento. El estrato y la probabilidad de selección acompañan cada caso cuando las fracciones de muestreo difieren. Los diagnósticos de desempeño distinguen la muestra de evaluación de los casos revisados selectivamente por ambigüedad.
