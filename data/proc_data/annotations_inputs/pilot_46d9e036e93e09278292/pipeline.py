...
PIPELINE_VERSION = "llm-pilot-1.7.0"
MAX_WORKERS = 32
...
PROVIDERS = frozenset({"openai", "bedrock"})
DEFAULT_BEDROCK_REGION = "us-west-2"
BEDROCK_MODEL_RE = re.compile(r"^(?:global|us|in)\.openai\.[a-z0-9.-]+$")
...
def prepare_run(service: ValidationService, selected: list[dict], sampling: dict,
                ...
                provider_region: str | None = None) -> Path:
    ...
    if provider not in PROVIDERS:
        raise ValueError("provider debe ser openai o bedrock")
    if provider == 'bedrock':
        if not BEDROCK_MODEL_RE.fullmatch(model):
            raise ValueError("Bedrock requiere un ID de perfil de inferencia OpenAI")
        if not provider_region or not re.fullmatch(r'[a-z]{2}-[a-z]+-\d', provider_region):
            raise ValueError("Bedrock requiere una región AWS válida")
    elif provider_region is not None:
        raise ValueError("provider_region solo corresponde a Bedrock")
    ...
    spec = {
        "pipeline_version": PIPELINE_VERSION, "provider": provider,
...
def _authorize_api_execution(input_run_dir: Path, result_dir: Path,
    ...
    if (
        grant.get("provider", "openai") != spec.get("provider", "openai")
        or grant.get("provider_region") != spec.get("provider_region")
        or grant.get("model") != spec.get("model")
...
def _run_annotations(input_run_dir: Path, result_dir: Path, execute: bool, workers: int,
    ...
    if execute and pending:
        ...
        provider = manifest['spec'].get('provider', 'openai')
        if provider == 'bedrock':
            api_key = os.environ.get('AWS_BEDROCK_API_KEY')
            region = manifest['spec'].get('provider_region')
            if not api_key or not region:
                raise ValidationError('Falta AWS_BEDROCK_API_KEY o la región congelada')
            base_url = f'https://bedrock-runtime.{region}.amazonaws.com/openai/v1'
        elif provider == 'openai':
            api_key = os.environ.get('OPENAI_API_KEY')
            if not api_key:
                raise ValidationError('Falta OPENAI_API_KEY')
            base_url = 'https://api.openai.com/v1'
        else:
            raise ValidationError(f'Proveedor no admitido: {provider}')
        ...
        def annotate(index: int) -> dict | None:
            ...
            result = {'sample_index': index, 'unit_id': request['unit_id'],
                      'provider': provider,
                      'model_requested': manifest['spec']['model'],
...
def export_results(input_run_dir: Path,
    ...
    for i, record in enumerate(records):
        ...
        row.update(sample_index=i, status=result['status'],
                   provider=result.get('provider', spec.get('provider', 'openai')),
                   model_requested=result.get('model_requested', spec['model']),
        ...
        for annotation in annotations:
            spans.append({**{k: row[k] for k in ['sample_index','unit_id','utterance_id','law_number','document_uri','provider','model_requested']},
    ...
    span_frame = pd.DataFrame(spans, columns=['sample_index','unit_id','utterance_id','law_number',
        'document_uri','provider','model_requested','annotation_id','concept_status','concept_id','proposed_concept','stance',
