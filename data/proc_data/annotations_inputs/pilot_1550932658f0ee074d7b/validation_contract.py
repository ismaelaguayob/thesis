...
def sample_with_design(
    ...
    if sample_size < len(groups):
        raise ValidationError(
            "El tamaño de muestra debe ser al menos igual al número de estratos "
            f"no vacíos ({len(groups)}) para asegurar su representación"
        )

    # Guarantee coverage first. The remaining places are apportioned by the
    # number of units left in each stratum, so a singleton stratum cannot take
    # an extra place that it does not have.
    quotas = {identifier: 1 for identifier in groups}
    remaining = sample_size - len(groups)
    residual_capacity = {
        identifier: counts[identifier] - 1 for identifier in groups
    }
    residual_total = sum(residual_capacity.values())
    ideal = {
        identifier: (
            residual_capacity[identifier] * remaining / residual_total
            if residual_total else 0
        )
        for identifier in groups
    }
    additions = {
        identifier: min(residual_capacity[identifier], math.floor(ideal[identifier]))
        for identifier in groups
    }
    for identifier, addition in additions.items():
        quotas[identifier] += addition
    remaining -= sum(additions.values())
    order = sorted(
        groups,
        key=lambda identifier: (
            -(ideal[identifier] - additions[identifier]),
