...
1. Actores: las personas que emiten la declaración en Sala, incluidos parlamentarios, integrantes del Ejecutivo y autoridades de las cámaras. Se registrarán su afiliación partidaria, género y edad cuando los datos estén disponibles. La edad se calculará a la fecha de la intervención y la afiliación partidaria se verificará para ese momento, dado que la extracción actual conserva la última militancia disponible en la BCN.
2. Conceptos: las justificaciones normativas que conformarán los nodos conceptuales de la red. El libro de códigos, disponible en el material complementario, se construyó a partir de los criterios CARIN y de categorías justificativas del caso chileno. Se revisó mediante rondas de calibración manual y se sometió a un piloto de aplicación con un LLM para examinar sus definiciones y reglas de uso.
3. Acuerdo: variable dicotómica que registra la orientación de cada declaración frente a la proposición de referencia del concepto. Toma el valor 1 cuando el actor la respalda y 0 cuando la rechaza o refuta. La orientación se determina respecto del concepto, con independencia del voto o la posición general del actor frente a la reforma.
4. Tiempo: cada declaración se asociará con la ley, la fecha, la sesión y el trámite constitucional en que fue emitida. Dentro de cada sesión, las intervenciones mantienen el orden original del debate.
...
El procedimiento de validación se realizó en tres etapas iterativas. En primer lugar, el libro de códigos fue evaluado en múltiples rondas de codificación manual por parte del investigador, utilizando muestras estratificadas por sesión y longitud. En segundo lugar, se realizaron iteraciones piloto con el LLM, revisando manualmente las anotaciones realizadas por el modelo, en particular las marcadas como ambigüas. Para finalizar, luego de la codificación definitiva se coidificará manualmente una muestra estratificada por ley, cámara, partido, género y tipo de actor (Bosley, 2025), reportando precisión, F1, y kappa de Cohen para la codificación de conceptos y postura y de estrategia de legitimación. Los casos ambiguos se adjudicarán manualmente, conservando su estado de revisión, y se examinará si se concentran por ley, tipo de actor, género o partido. Los casos irresolubles no se convertirán en una categoría sustantiva. Si una tarea o categoría no alcanza un desempeño suficiente, se revisará el instrumento y se repetirá su evaluación antes de escalar el procesamiento (Crupi et al., 2025; Dunivin, 2025). El contrato de salida, las reglas de adjudicación y los diagnósticos detallados se conservarán en el material suplementario.
...
Una vez identificadas las coaliciones, se analizarán sus estrategias de legitimación. Esta secuencia separa las posiciones sustantivas que constituyen las coaliciones del modo en que cada grupo las presenta como aceptables, como se ha hecho en estudios que combinan DNA y estrategias narrativas (Schaub, 2021). Para responder Q2, la unidad de análisis del modelo será el segmento justificativo con una estrategia de legitimación resuelta; la categoría asignada será la variable dependiente. Se estimará un modelo logístico multinomial multinivel, cuyos principales predictores serán la coalición discursiva, la filiación partidaria, el tipo de actor y la etapa legislativa. Dado que un mismo actor produce múltiples segmentos justificativos en distintas sesiones y que cada sesión reúne a varios actores, se incorporarán interceptos aleatorios cruzados para actor y sesión (Tranmer et al., 2014). Los fragmentos sin segmentos de legitimación explícita se describirán por separado. El vínculo de cada segmento con su fragmento de origen permitirá describir la coexistencia de estrategias dentro de una misma unidad textual. Se informarán probabilidades predichas para describir cómo varía el uso de las estrategias entre coaliciones, sin imponer una expectativa direccional previa. El tratamiento de casos irresolubles y los análisis de sensibilidad se detallarán en el material suplementario metodológico.
